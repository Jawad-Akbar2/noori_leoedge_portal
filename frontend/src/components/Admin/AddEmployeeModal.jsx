...
export default function AddEmployeeModal({ onClose, onSave, currentUserRole }) {
  ...
  const calculateMonthlySalary = () => {
    ...
    const workMins = Math.max(0, endMin - startMin - breakMins);
    return (
      (workMins / 60) *
      30 *
  ...
  uploads.push(
    axios.put(
      ...
      {
        idCard: {
          data: idCard.front.url,
          fileName: idCard.front.fileName,
          mimeType: idCard.front.mimeType,
        },
      },
      { headers: { Authorization: `Bearer ${token}` } },
    ),
...
if (idCard.back?.url) {
  uploads.push(
    axios.put(
      ...
      {
        idCard: {
          data: idCard.back.url,
          fileName: idCard.back.fileName,
          mimeType: idCard.back.mimeType,
        },
      },
      { headers: { Authorization: `Bearer ${token}` } },
    ),
                ...
                <div className="grid grid-cols-2 gap-4">
                  ...
                  <div>
                    ...
                    <p className="text-xs text-gray-500 mt-1">
                      Unpaid break deducted from scheduled hours (default: 60
                      min)
                    </p>
                  </div>
                </div>
  ...
  onChange={(e) => {
    setFormData(prev => ({ ...prev, salaryType: e.target.value }));
    // Don't clear monthlySalary/hourlyRate on switch
  }}
  disabled={loading}
  className="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 disabled:bg-gray-100"
>
                  ...
                  {formData.salaryType === "hourly" ? (
                    <>
                      ...
                      <p className="text-xs text-gray-500 mt-2">
                        ...
                        PKR {formData.hourlyRate}/hr × 30 days
                      </p>
