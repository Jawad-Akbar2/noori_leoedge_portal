...
export default function EditEmployeeModal({ employee, onClose, onSave, currentUserRole }) {
  ...
  const [formData, setFormData] = useState({
    ...
    shift: { start: "09:00", end: "18:00", break: 60 },
...
const shiftWorkHours = () => {
  if (!formData.shift.start || !formData.shift.end) return 7; // default 7h
  ...
  const breakMins = Number(formData.shift.break ?? 60);
  return Math.max(1, (end - start - breakMins) / 60);
};

useEffect(() => {
  if (formData.salaryType === 'monthly' && formData.monthlySalary) {
    const hrs = shiftWorkHours();
    const derived = parseFloat(formData.monthlySalary) / 30 / hrs;
    if (derived > 0) {
      setFormData(prev => ({ ...prev, hourlyRate: Math.round(derived * 100) / 100 }));
    }
  }
}, [formData.monthlySalary, formData.salaryType]);

useEffect(() => {
  if (formData.salaryType === 'hourly' && formData.hourlyRate) {
    const hrs = shiftWorkHours();
    const derived = parseFloat(formData.hourlyRate) * hrs * 30;
    if (derived > 0) {
      setFormData(prev => ({ ...prev, monthlySalary: Math.round(derived) }));
    }
  }
}, [formData.hourlyRate, formData.salaryType]);
    ...
    (async () => {
      ...
      try {
        ...
        setFormData({
          ...
          shift: emp.shift || { start: '09:00', end: '18:00', break: 60 },
...
const handleIdDelete = async (side) => {
  ...
  try {
    const token = localStorage.getItem('token');
    await axios.delete(                                    // ← use dedicated DELETE route
      `/api/employees/${employee._id}/id-card/${side}`,
      { headers: { Authorization: `Bearer ${token}` } }
    );
 ...
 const calculateMonthlyEstimate = () => {
  ...
  if (end <= start) end += 24 * 60;
  const breakMins = Number(formData.shift.break ?? 60);
  const workMins = Math.max(0, (end - start) - breakMins);
  return (workMins / 60 * 30 * parseFloat(formData.hourlyRate))
...
<Field label="Break Duration (minutes)">
  ...
  <p className="text-[10px] text-gray-400 mt-1">
    Unpaid break deducted from scheduled hours (default: 60 min)
  </p>
</Field>
                  </div>
  ...
  onClick={() => {
    setFormData(prev => ({ 
      ...prev, 
      salaryType: 'hourly',
      // Don't wipe monthlySalary so switching back keeps it
    }));
...
>
  ...
  onClick={() => {
    setFormData(prev => ({ 
      ...prev, 
      salaryType: 'monthly',
      // Don't wipe hourlyRate so switching back keeps it
    }));
  ...
  <div className="space-y-2">
    <div>
      ...
      <p className="text-2xl font-bold text-blue-700">
        PKR {Number(formData.monthlySalary || 0).toLocaleString("en-PK", {
          minimumFractionDigits: 0, maximumFractionDigits: 0,
        })}
      </p>
    </div>
    <div>
      <p className="text-[10px] text-gray-400 uppercase tracking-wide mb-0.5">Hourly Equivalent</p>
      <p className="text-sm font-semibold text-gray-600">
        PKR {Number(formData.hourlyRate || 0).toLocaleString("en-PK", {
          minimumFractionDigits: 2, maximumFractionDigits: 2,
        })}/hr
      </p>
    </div>
    <p className="text-[10px] text-gray-400">
      Based on {shiftWorkHours().toFixed(1)}h/day × 30 days
    </p>
    {formData.salaryType === 'hourly' && (
      <p className="text-[10px] text-amber-600 flex items-center gap-1">
        <AlertCircle size={10} /> Estimate only — actual pay depends on attendance
      </p>
    )}
  </div>
</InfoCard>
