...
import {
  ...
  Home, Phone, FileText, Upload, Trash2, Camera, BadgeCheck
} from 'lucide-react';
...
const ROLE_CONFIG = {
  superadmin: {
    ...
    canEditPersonal: true, canEditEmail: true, canEditBank: true,
    canEditShift: true, canEditSalary: true, canEditStatus: true,
    canEditJoining: true, canEditDepartment: true,
  },
  admin: {
    label: 'Administrator',
    Icon: ShieldCheck,
    ...
    canEditEmail: true, canEditBank: true,
  },
  employee: {
    label: 'Employee',
    Icon: UserCircle,
    ...
    pwBtnBg: 'bg-emerald-50 text-emerald-700 border-emerald-200 hover:bg-emerald-100',
    canEditEmail: true, canEditBank: true,
  },
  hybrid: {
...
const ACCENT_CLASSES = {
  indigo: { ring: 'focus:ring-indigo-500', border: 'border-indigo-300', bg: 'bg-indigo-50/40', headerGrad: 'from-indigo-700 to-indigo-500', avatarRing: 'ring-indigo-400' },
  blue:   { ring: 'focus:ring-blue-500',   border: 'border-blue-300',   bg: 'bg-blue-50/30',   headerGrad: 'from-blue-700 to-blue-500',   avatarRing: 'ring-blue-400' },
  emerald:{ ring: 'focus:ring-emerald-500',border: 'border-emerald-300',bg: 'bg-emerald-50/30',headerGrad: 'from-emerald-700 to-emerald-500',avatarRing: 'ring-emerald-400' },
  purple: { ring: 'focus:ring-purple-500', border: 'border-purple-300', bg: 'bg-purple-50/30', headerGrad: 'from-purple-700 to-purple-500', avatarRing: 'ring-purple-400' },
};
...
const Card = ({ icon: Icon, iconBg = 'bg-blue-50', iconColor = 'text-blue-600', title, badge, children, optional }) => (
  <div className="bg-white rounded-xl shadow-sm border border-gray-100 p-6">
    <div className="flex items-center justify-between mb-5 pb-4 border-b border-gray-100">
      <div className="flex items-center gap-2.5">
        <div className={`p-2 rounded-lg ${iconBg}`}><Icon size={18} className={iconColor} /></div>
        <h2 className="text-base font-bold text-gray-800">{title}</h2>
        {optional && <span className="text-xs text-gray-400 bg-gray-100 px-2 py-0.5 rounded-full ml-1">Optional</span>}
      </div>
      {badge && <span className="text-xs font-medium text-gray-400 bg-gray-100 px-2.5 py-1 rounded-full">{badge}</span>}
    </div>
    {children}
  </div>
);
...
const IdCardSide = ({ label, side, currentFile, onUpload, onDelete, accentBorder, accentRing, isLoading, globalBusy }) => {
  ...
  const handleFileSelect = (e) => {
    ...
    if (!validTypes.includes(file.type)) { toast.error('JPEG, PNG, GIF, WebP or PDF only'); return; }
      ...
      >
        ...
        {currentFile?.url ? (
          ...
          <div className="flex flex-col items-center justify-center h-36 gap-2 text-gray-400">
            ...
            <span className="text-[10px] text-gray-400">JPEG, PNG, PDF · max 5MB</span>
          </div>
        )}
      </div>
      <input
...
export default function MyProfile() {
  ...
  const { Icon, canEditPersonal, canEditEmail, canEditBank, btnBg, pwBtnBg } = config;
  ...
  const [form, setForm]           = useState({
    ...
    status: 'Active', joiningDate: '', employeeNumber: '',
  ...
  const fetchProfile = async () => {
    try {
      ...
      const { data } = await axios.get('/api/employees/me', { headers: { Authorization: `Bearer ${token}` } });
      if (data.success) {
        ...
        setForm({
          ...
          bankName: emp.bank?.bankName || '', accountName: emp.bank?.accountName || '',
          accountNumber: emp.bank?.accountNumber || '',
          shiftStart: emp.shift?.start || '', shiftEnd: emp.shift?.end || '',
          salaryType: emp.salaryType || 'hourly', hourlyRate: emp.hourlyRate || '',
          monthlySalary: emp.monthlySalary || '', status: emp.status || 'Active',
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      const basePayload = {
        ...
        bank: { bankName: form.bankName.trim(), accountName: form.accountName.trim(), accountNumber: form.accountNumber.trim() },
      ...
      if (role === 'superadmin') {
        const res = await axios.put(
          `/api/employees/${employee._id}`,
          {
            ...
            salaryType: form.salaryType, employeeNumber: form.employeeNumber.trim(),
            hourlyRate: parseFloat(form.hourlyRate) || 0,
            monthlySalary: form.salaryType === 'monthly' ? parseFloat(form.monthlySalary) || null : null,
            status: form.status, joiningDate: form.joiningDate,
          },
          { headers: { Authorization: `Bearer ${token}` } }
        );
        data = res.data;
      } else {
        const res = await axios.put('/api/employees/me', basePayload, { headers: { Authorization: `Bearer ${token}` } });
        data = res.data;
      }

      if (data.success) { setEmployee(data.employee); toast.success('Profile saved'); }
  ...
  const SalaryRows = () => {
    ...
    return employee.salaryType === 'monthly' ? (
      <><InfoBox label="Salary Type" value="Monthly" /><InfoBox label="Monthly Salary (PKR)" value={employee.monthlySalary?.toLocaleString('en-PK')} /></>
    ) : (
      <><InfoBox label="Salary Type" value="Hourly" /><InfoBox label="Hourly Rate (PKR)" value={employee.hourlyRate?.toLocaleString('en-PK')} /></>
    );
  };
    ...
    return (
      <div className="flex items-center justify-center min-h-screen bg-gray-50">
        <div className="flex flex-col items-center gap-3">
          <div className={`w-8 h-8 border-2 border-t-transparent rounded-full animate-spin border-${config.accent}-500`} />
      ...
      <div className={`w-full bg-gradient-to-br ${ac.headerGrad} text-white`}>
        <div className="max-w-5xl mx-auto px-4 md:px-8 py-8 flex flex-col sm:flex-row items-center sm:items-end gap-6">
          ...
          <div className="flex-1 min-w-0 order-2 sm:order-1 text-center sm:text-left">
            ...
            <h1 className="text-3xl font-extrabold tracking-tight truncate">{fullName || '—'}</h1>
            <p className="text-white/80 text-sm mt-1 truncate">{employee?.email}</p>
            <div className="flex flex-wrap gap-3 mt-3 justify-center sm:justify-start">
              ...
              {employee?.status && (
                <span className={`inline-flex items-center gap-1.5 px-3 py-1 rounded-full text-xs font-medium
                  ${employee.status === 'Active'   ? 'bg-green-400/30 text-green-100' :
                    employee.status === 'Frozen'   ? 'bg-blue-400/30 text-blue-100'   :
          ...
          <div className="space-y-4">
            ...
            {employee?.salaryType && (
              config.canEditSalary ? (
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4"><SalaryRows /></div>
              )
            )}
          </div>
        </Card>
            ...
            <form onSubmit={handleChangePassword} className="space-y-4">
              {[
                ...
                <div key={field}>
                  ...
                  {key === 'newPassword' && <p className="text-xs text-gray-400 mt-1">Must be at least 8 characters</p>}
                </div>
              ))}
