...
const employeeSchema = new mongoose.Schema({
  ...
  status:     { type: String, enum: ['Active', 'Inactive', 'Frozen'], default: 'Inactive', index: true },
...
employeeSchema.pre('validate', function (next) {
  if (SYSTEM_ROLES.includes(this.role)) {
    ...
    if (this.shift) { this.shift.start = null; this.shift.end = null; }
    return next();
  }
  ...
  if (PAYROLL_ROLES.includes(this.role)) {
    ...
    if (!this.salaryType) {
      ...
      if (!this.hourlyRate || this.hourlyRate <= 0) errors.push('hourlyRate is required and must be > 0');
