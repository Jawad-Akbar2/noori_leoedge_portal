...
router.post('/leave/submit', employeeAuth, async (req, res) => {
  try {
    const { fromDate, toDate, leaveType, reason, isNewEmployeeRequest } = req.body;
   ...
   if (daysElapsed < 90) {
  if (!isNewEmployeeRequest) {
    // Employee hasn't acknowledged they're applying early
    return res.status(400).json({
      success: false,
      message: `Leave not eligible yet. ${90 - daysElapsed} day(s) remaining.`,
      daysUntilEligible: 90 - daysElapsed,
      requiresNewEmployeeAck: true,   // ← frontend uses this to show the checkbox
    });
  }
    }
    ...
    const leaveRequest = new LeaveRequest({
      ...
      isNewEmployeeRequest: isNewEmployeeRequest && daysElapsed < 90,
    });
...
// ─── GET /api/requests/leave/quota  (employee) ────────────────────────────────
router.get('/leave/quota', employeeAuth, async (req, res) => {
  try {
    ...
    const yearStart = new Date(joining);
    ...
    const QUOTA = { 'Sick Leave': 7, 'Holiday Leave': 13 };
    const spent = { 'Sick Leave': 0, 'Holiday Leave': 0 };

    for (const l of leaves) {
      if (spent[l.leaveType] !== undefined) {
        spent[l.leaveType] += l.totalDays || 1;
      }
    }
    ...
    return res.json({
      ...
      spent,
      remaining: {
        'Sick Leave':    Math.max(0, QUOTA['Sick Leave']    - spent['Sick Leave']),
        'Holiday Leave': Math.max(0, QUOTA['Holiday Leave'] - spent['Holiday Leave']),
      },
