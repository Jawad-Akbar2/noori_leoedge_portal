...
function resolveNightShiftCheckoutDate(outTime, csvDate, shift) {
  ...
  if (!isNightShift) return { shiftDate: csvDate, outNextDay: false };
  const outMin = toMin(outTime);
  const shiftStartMin = toMin(shift.start);
  if (outMin >= shiftStartMin) return { shiftDate: csvDate, outNextDay: false };
...
function applyShiftBasedPairing(shiftStart, punchTimes) {
  if (!punchTimes || punchTimes.length === 0) return { inTime: null, outTime: null, outNextDay: false };
  ...
  if (!inEntry) {
    ...
    if (outOnly) return { inTime: null, outTime: outOnly.time, outNextDay: outOnly.norm > 1440 || toMin(outOnly.time) < shiftStartMin % 1440 };
    ...
    try {
      ...
      if (parsed.length === 0) {
        return res.status(400).json({
          success: false, message: "No valid rows found in CSV file",
          processingLog: log,
      ...
      for (const [, groupData] of Object.entries(grouped)) {
        ...
        if (isNightShiftEmp && inTime && outTime && outNextDay) {
          ...
          if (outMin < shiftStartMin) {
            ...
            log.push({ type: "INFO", message: `  ℹ️ Mixed group: out=${outTime} → prev shift ${formatDate(prevDay)}, in=${inTime} → today ${dateStr}` });
            ...
            try {
              ...
              if (prevExisting && prevExisting.manualOverride) {
                log.push({ type: "WARN", message: `  ⚠️ Prev shift (${formatDate(prevDay)}) has manual override — checkout skipped` });
              } else if (prevExisting) {
                const prevMergedOut = latestTime(prevExisting.inOut?.out, outTime);
                const prevMergedIn = prevExisting.inOut?.in || null;
                const prevOutNextDay = prevMergedIn && prevMergedOut ? toMin(prevMergedOut) < toMin(prevMergedIn) : true;
                const prevStatus = prevMergedIn && isLate(prevMergedIn, employee.shift.start) ? "Late" : "Present";
                const prevFinancials = buildFinancials({
                  status: prevStatus, inTime: prevMergedIn, outTime: prevMergedOut,
                  outNextDay: prevOutNextDay, shift: employee.shift, hourlyRate: prevRate,
                ...
                await AttendanceLog.updateOne(
                  ...
                  { $set: {
                    status: prevStatus, "inOut.out": prevMergedOut,
                    "inOut.outNextDay": prevOutNextDay, financials: prevFinancials,
                    "metadata.source": "csv", "metadata.lastUpdatedBy": req.userId,
                ...
                // ✅ prev-shift create — uses findOneAndUpdate to handle soft-deleted slot
                const prevFinancials = buildFinancials({
                  status: "Present", inTime: null, outTime, outNextDay: true,
                  shift: employee.shift, hourlyRate: prevRate, salaryType: employee.salaryType,
                });
                ...
                recordsCreated++;
                log.push({ type: "SUCCESS", message: `  ✓ Prev shift (${formatDate(prevDay)}) created with out=${outTime}` });
              }
            } catch (splitErr) {
        ...
        if (!inTime && outTime) {
          ...
          if (resolved.shiftDate.getTime() !== date.getTime()) {
            log.push({ type: "INFO", message: `  ℹ️ Night-shift checkout — resolved to shift date: ${formatDate(resolved.shiftDate)}` });
          }
        }
        ...
        try {
          ...
          if (existing) {
            ...
            let mergedOutNextDay = existing.shift?.isNightShift && mergedIn && mergedOut
            ...
            await AttendanceLog.updateOne(
              { _id: existing._id },
              { $set: {
                status: mergedStatus, salaryType: employee.salaryType,
                "inOut.in": mergedIn || null, "inOut.out": mergedOut || null,
                "inOut.outNextDay": mergedOutNextDay, hourlyRate: rate,
                financials: mergedFinancials, "metadata.source": "csv",
                "metadata.lastUpdatedBy": req.userId, "metadata.lastModifiedAt": new Date(),
              }},
            );
            recordsUpdated++;
            log.push({ type: "SUCCESS", message: `  ✓ Updated (merged — ${mergedStatus})` });
          } else {
            // ✅ main create — uses findOneAndUpdate to handle soft-deleted slot
            await AttendanceLog.findOneAndUpdate(
              { empId: employee._id, date: attendanceDate },
              { $set: {
                empNumber: employee.employeeNumber,
                empName: `${employee.firstName} ${employee.lastName}`,
                department: employee.department,
      ...
      return res.json({
        success: true, message: "CSV import complete", processingLog: log,
      ...
      return res.status(500).json({
        success: false, message: "Error processing CSV file", error: err.message, processingLog: log,
...
router.post("/worksheet", adminAuth, async (req, res) => {
  try {
    ...
    const logs = await AttendanceLog.find({
      empId: { $in: empIds }, date: { $gte: start, $lte: end }, isDeleted: false,
    }).lean();
    ...
    for (let d = new Date(start); d <= end; d.setDate(d.getDate() + 1)) {
      ...
      for (const emp of employees) {
        ...
        if (existing) {
          worksheet.push({
            _id: existing._id, date: disp, dateRaw: existing.date,
            empId: emp._id, empNumber: emp.employeeNumber,
            empName: `${emp.firstName} ${emp.lastName}`, department: emp.department,
            shift: emp.shift, salaryType: existing.salaryType || emp.salaryType,
            hourlyRate: existing.hourlyRate, status: existing.status, inOut: existing.inOut,
            financials: { ...existing.financials, deductionDetails: existing.financials?.deductionDetails || [], otDetails: existing.financials?.otDetails || [] },
            ...
            isVirtual: false, isModified: false,
          });
        } else {
          worksheet.push({
            date: disp, dateRaw: new Date(iso),
            empId: emp._id, empNumber: emp.employeeNumber,
            empName: `${emp.firstName} ${emp.lastName}`, department: emp.department,
            shift: emp.shift, salaryType: emp.salaryType,
            hourlyRate: effectiveHourlyRate(emp, 26), status: "Absent",
            inOut: { in: null, out: null, outNextDay: false },
            financials: { hoursWorked: 0, scheduledHours: shiftHours(emp.shift), lateMinutes: 0, earlyLogoutMinutes: 0, basePay: 0, deduction: 0, deductionDetails: [], otMultiplier: 1, otHours: 0, otAmount: 0, otDetails: [], finalDayEarning: 0 },
            manualOverride: false, lastModified: "--", lastModifiedRaw: null,
            isVirtual: true, isModified: false,
          });
        }
      }
    }
...
router.post("/save-row", adminAuth, async (req, res) => {
  try {
    ...
    // 🚨 Block attendance after employee left
    ...
    // ✅ Only look at active (non-deleted) record for OT preservation
    const existingRecord = await AttendanceLog.findOne({
      empId: employee._id, date: dateObj, isDeleted: false,
    });
    ...
    const financials = buildFinancials({
      status, inTime: resolvedInTime, outTime: resolvedOutTime,
      outNextDay: resolvedOutNextDay, shift: employee.shift,
      hourlyRate: rate, salaryType: employee.salaryType,
      ...preservedOt,
    });
    ...
    // ✅ Resolve record: active → soft-deleted (resurrect) → new
    ...
    record.isDeleted = false; // ✅ always reset — resurrects soft-deleted records
...
router.post("/bulk-save", adminAuth, async (req, res) => {
  try {
    ...
    for (const row of rows) {
      ...
      const rowOtDetails = (Array.isArray(row.financials?.otDetails) ? row.financials.otDetails : []).map((e) => ({ ...e, type: "manual" }));
